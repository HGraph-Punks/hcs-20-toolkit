  ...
  const handleFileChange = async (event) => {
    ...
    try {
        const response = await axios({
            method: 'post',
            url: '/api/compressAndChunkImage', // Adjust the URL based on your API route
            data: formData,
            headers: { 'Content-Type': 'multipart/form-data' },
        });

        if (response.data.success) {
            // Assuming the API returns the base64 chunks in an array called 'chunks'
            const base64Image = response.data.chunks.join('');
            setImageData(`data:${file.type};base64,${base64Image}`);
            
            // Proceed with any additional logic, such as setting progress, cost, etc.
