...
//       ...metaData,
//       image: new File([file], `${metaData?.name}${'.' + file.name.split('.')[1]}`, {
//         type: file.type,
//       })
//     })
...
export const createIPFSMetaData = async (hashLipsImages, hashlipsMetaData, nftStorageAPI) => {
  return new Promise(async (resolve, reject) => {
    ...
    hashlipsMetaData=hashlipsMetaData.reverse()
    // sortedHashLipsImagesArray.reverse()
    // // Crate Array for CIDs
    let metaDataCIDs = [];
      for (let index = 0; index < hashlipsMetaData.length; index++) {
        const fileType = () => {
          console.log('.'+sortedHashLipsImagesArray[index].name.split('.')[1])
          return '.'+sortedHashLipsImagesArray[index].name.split('.')[1]
        }
        const fileName = () => {
          console.log(sortedHashLipsImagesArray[index].name.split('.')[0])
          debugger
          return sortedHashLipsImagesArray[index].name.split('.')[0]
        }
        const metadata = await nftStorageClient.store({
          ...hashlipsMetaData[index],
          image: new File([sortedHashLipsImagesArray[index]], `${fileName()}${fileType()}`, {
            type: sortedHashLipsImagesArray[index].type,
          })
        })
        metaDataCIDs.push(metadata.url);
        if (metaDataCIDs.length -1 === hashlipsMetaData.length -1) {
          const finalMetadataCIDS = metaDataCIDs.reverse()
          resolve(finalMetadataCIDS)
        }
      }
  });
...
export const createHCSMetaData = async (hashLipsImages, hashlipsMetaData, client) => {
  return new Promise(async (resolve, reject) => {
...
export const uploadFile = async (event, token, setToken) => {
  ...
  setToken({
      ...
      imageType: '.' + media.name.split('.')[1],
      photoSize: media.size,
  })
}
